...
namespace BIGINT
{
        ...
        else
        {
            ...
            try
            {
                for (int i = str.length() - 1;i >= end;i--)
                {
                    ...
                    {
                        throw std::runtime_error("BigInt:Initial has ilegal char: ");
                    }
                }
            }
            catch (std::runtime_error &err)
            {
                std::cout << err.what() << str << std::endl << "Plz try again." << std::endl;
                data.clear();
                is_zero = true;
                sign = 1;
                return;
    ...
    //将大整数转为字符串
    const std::string BigInt::ToString(void) const
    {
        if (is_zero)
        {
            return std::string("0");
